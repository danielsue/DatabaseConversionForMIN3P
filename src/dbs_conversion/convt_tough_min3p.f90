...
contains    
    ...
    subroutine convert2Min3PAqueousSpecies
        ...
        do i = 1, nAqueousSpecies
            ...
            if(aqueousSpecies(i)%bRedoxReaction) then       !Redox reaction
                ...
                !.....................................
                min3PRedoxReactions(j)%NCP = min(aqueousSpecies(i)%NCP,5)
                
                m = min3PRedoxReactions(j)%NCP
               
                if (aqueousSpecies(i)%NCP > 5) then
                    call WriteLog("Warning:"// "Number of components exceed 5, only 5 components are converted.") 
                ...
                !
                min3PComplexReactions(k)%NCP = min(aqueousSpecies(i)%NCP,5)
    ...
    subroutine convert2Min3PGases
        ...
        do i = 1, nGases
            ...
            !
            min3PGases(i)%NCP = min(gases(i)%NCP,5)
                
            j = min3PGases(i)%NCP
               
            if (gases(i)%NCP > 5) then
                call WriteLog("Warning:"// "Number of components exceed 5, only 5 components are converted.") 
                nWarnings = nWarnings + 1
            end if
    ...
    subroutine convert2Min3PMinerals
        ...
        do i = 1, nMinerals
            ...
            min3PMinerals(i)%NCP = min(minerals(i)%NCP,5)
                
            j = min3PMinerals(i)%NCP
               
            if (minerals(i)%NCP > 5) then
                call WriteLog("Warning:"// "Number of components exceed 5, only 5 components are converted.") 
                nWarnings = nWarnings + 1
            end if
